...
def get_model_and_assets_general(mode):
	"""Returns a tuple containing the model XML string and a dict of assets."""
	return resources.GetResource(os.path.join(_TASKS_DIR, f'hopper_variants/hopper_{mode}.xml')), common.ASSETS
...
@hopper.SUITE.add('custom')
def three_hop(time_limit=_DEFAULT_TIME_LIMIT, random=None, environment_kwargs=None):
  """Returns a Hopper that strives to hop forward."""
  physics = Physics.from_xml_string(*get_model_and_assets_general(3))
  task = hopper.Hopper(hopping=True, random=random)
  environment_kwargs = environment_kwargs or {}
  return control.Environment(
      physics, task, time_limit=time_limit, control_timestep=_CONTROL_TIMESTEP,
      **environment_kwargs)
...
@hopper.SUITE.add('custom')
def four_hop(time_limit=_DEFAULT_TIME_LIMIT, random=None, environment_kwargs=None):
  """Returns a Hopper that strives to hop forward."""
  physics = Physics.from_xml_string(*get_model_and_assets_general(4))
  task = hopper.Hopper(hopping=True, random=random)
  environment_kwargs = environment_kwargs or {}
  return control.Environment(
      physics, task, time_limit=time_limit, control_timestep=_CONTROL_TIMESTEP,
...
@hopper.SUITE.add('custom')
def three_hop_backwards(time_limit=_DEFAULT_TIME_LIMIT, random=None, environment_kwargs=None):
	"""Returns the Hop Backwards task."""
	physics = Physics.from_xml_string(*get_model_and_assets_general(3))
	task = CustomHopper(goal='hop-backwards', random=random)
	environment_kwargs = environment_kwargs or {}
...
@hopper.SUITE.add('custom')
def four_hop_backwards(time_limit=_DEFAULT_TIME_LIMIT, random=None, environment_kwargs=None):
	"""Returns the Hop Backwards task."""
	physics = Physics.from_xml_string(*get_model_and_assets_general(4))
	task = CustomHopper(goal='hop-backwards', random=random)
	environment_kwargs = environment_kwargs or {}
...
@hopper.SUITE.add('custom')
def three_stand(time_limit=_DEFAULT_TIME_LIMIT, random=None, environment_kwargs=None):
...
@hopper.SUITE.add('custom')
def four_stand(time_limit=_DEFAULT_TIME_LIMIT, random=None, environment_kwargs=None):
  """Returns a Hopper that strives to stand upright, balancing its pose."""
  physics = Physics.from_xml_string(*get_model_and_assets_general(4))
  task = Hopper(hopping=False, random=random)
  environment_kwargs = environment_kwargs or {}
